...
public class TimerTest {
    ...
    public void resetTimer() {
        ...
        timer.resetTimer();
        assertEquals(
                timer.getCompositeTime() + timer.getDemographicsTime()
                        + timer.getIc1Time() + timer.getIc2Time()
                        + timer.getLoginTime() + timer.getOverviewTime()
                        + timer.getSideBySideTime(), 0);
    }

    @Test
    public void formattingDate() {
        String input = "20140731_105047";
        System.out.println(getDate().substring(4, 6) + "-"
                + getDate().substring(6, 8) + "-" + getDate().substring(0, 4)
                + "  " + getDate().substring(9, 11) + ":"
                + getDate().substring(11, 13) + ":"
                + getDate().substring(13, 15));
    }
